    ...
    <nav className="flex items-center justify-between py-0 px-5 fixed top-0 left-0 right-0 z-10 bg-white">
      <div className="flex items-center flex-1 min-w-0">
        ...
        {(breadcrumbState === BreadcrumbState.ENTERING || breadcrumbState === BreadcrumbState.VISIBLE || breadcrumbState === BreadcrumbState.EXITING) && (breadcrumbData || selectedVendor || selectedCollection) && (
          <div className={`ml-3 flex items-center text-sm font-mono text-brutalist-grey flex-1 min-w-0 overflow-hidden ${
            ...
            {breadcrumbData ? (
              ...
              <>
                ...
                {breadcrumbData.collections && breadcrumbData.collections.length > 0 ? (
                  <>
                    <span className={`${isMobile ? 'mx-1' : 'mx-2'} text-brutalist-grey`}>/</span>
                    <button
                      onClick={() => {
                ...
                <button
                  ...
                  className={`text-brutalist-black font-medium transition-colors cursor-pointer hover:text-[#00b140] ${
        ...
        {isEventsPageFromPath && onTogglePastEvents && !isBackVisible && (
          <div className={`flex items-center ${isMobile ? 'space-x-1 ml-2' : 'space-x-2 ml-4'}`}>
            <button
              ...
              className={`bg-brutalist-white rounded-none font-mono hover:opacity-70 transition-opacity ${
                isMobile 
                  ? 'px-2 py-1 text-xs' 
                  : 'px-3 py-1 text-sm'
              } ${
      ...
      <div className="flex items-center">
        <button
          ...
          className={`relative z-20 size-12 flex items-center justify-center transition-colors duration-200 focus-interactive rounded-none ${
