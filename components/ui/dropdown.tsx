...
export function SelectDropdown({
  ...
  const trigger = (
    <div className={cn(
      'flex items-center justify-center gap-2 px-3 py-1 bg-brutalist-white font-mono text-sm uppercase hover:bg-brutalist-grey hover:bg-opacity-10 transition-colors min-w-[120px] cursor-pointer rounded-none',
      className
    )}>
      ...
      <span className="ml-2 text-brutalist-grey">▼</span>
    </div>
  );
      ...
      {options.map((option) => (
        ...
        >
          ...
          {option.count !== undefined && (
            <span className="text-brutalist-grey text-xs">({option.count})</span>
          )}
          {option.extra}
        </DropdownItem>
      ))}
