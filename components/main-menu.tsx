...
const getMenuAnimationClasses = (currentState: MenuState, isTopBar: boolean) => {
  const baseClasses = "absolute left-0 h-[2px] bg-black origin-center transition-all duration-300 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)]";
...
export function MainMenu({ isBackVisible, onBack, onMenuToggle }: MainMenuProps) {
  ...
  return (
    <div className="relative flex items-center">
      <button
        className="p-2 z-20 relative size-12 flex items-center justify-center focus-interactive rounded"
        onClick={handleClick}
            ...
            {['TRACKING', 'EVENTS'].map((item, index) => (
              ...
              >
                <Link
                  ...
                  className="text-sm font-mono hover:opacity-70 transition-opacity whitespace-nowrap focus-interactive rounded px-1 py-0.5"
                >
                  {item}
                </Link>
              </li>
