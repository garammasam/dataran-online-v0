      ...
      <Sheet open={isOpen} onOpenChange={onClose}>
        <SheetContent className="w-full sm:max-w-sm border-l-0 p-0 [&_button[aria-label=Close]]:hidden fixed right-0 top-0 h-[100dvh] max-h-[100dvh] overflow-hidden">
          ...
          <div className="flex flex-col h-full justify-center items-center px-6 text-center">
            ...
            <button
              onClick={onClose}
              className="absolute right-4 top-4 size-12 flex justify-center items-center rounded-sm opacity-70 ring-offset-background transition-all hover:opacity-100 focus-interactive"
            ...
            <div className="flex flex-col items-center space-y-6">
              <div className="size-16 bg-gray-100 rounded-sm flex items-center justify-center">
             
              </div>
              <div className="space-y-2">
                <h3 className="font-mono font-semibold text-lg uppercase tracking-wide">Cart Empty</h3>
                
              </div>
              <button
                onClick={onClose}
                className="font-mono font-semibold text-sm uppercase rounded-sm bg-black text-white px-6 py-3 transition-all duration-200 hover:bg-gray-800 focus-ring"
              >
                BROWSE
              </button>
            </div>
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-sm border-l-0 p-0 [&_button[aria-label=Close]]:hidden fixed right-0 top-0 h-[100dvh] max-h-[100dvh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-5 border-b bg-white">
              ...
              <div className="size-8 bg-[#00b140] rounded-sm flex items-center justify-center">
          ...
          <button
            onClick={onClose}
            className="size-10 flex justify-center items-center rounded-sm opacity-70 ring-offset-background transition-all hover:opacity-100 hover:bg-gray-100 focus-interactive"
            aria-label="Close cart"
          >
            <X className="size-5" />
          </button>
        ...
        <div className="flex flex-col h-full">
                  ...
                  >
                    <div
                      className={`p-5 transition-all duration-200 ${
                        hasStockIssue ? 'opacity-60' : 'opacity-100 hover:bg-gray-50'
                      }`}
                    >
                    <div className="flex gap-4">
                      {/* Product Image */}
                      <div className="relative aspect-square h-[90px] bg-white border border-gray-200 rounded-sm flex-shrink-0 overflow-hidden">
                        <Image
                      ...
                      <div className="flex-1 min-w-0 space-y-3">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="font-mono text-xs uppercase text-gray-500 tracking-wider">Price</span>
                          ...
                          <div className="space-y-1">
                            {item.selectedVariant.selectedOptions.map((option, index) => (
                              <div key={index} className="flex items-center justify-between">
                                <span className="font-mono text-xs uppercase text-gray-500 tracking-wider">{option.name}</span>
                        ...
                        {hasStockIssue && stockInfo && (
                          <div className="flex items-center justify-between">
                            <span className="font-mono text-xs uppercase text-gray-500 tracking-wider">Status</span>
                            <div className="text-right">
                        ...
                        <div className="flex items-center justify-between pt-2">
                          <span className="font-mono text-xs uppercase text-gray-500 tracking-wider">Quantity</span>
                          <div className="flex items-center border border-gray-300 rounded-sm bg-white">
                            <button
                              className="p-2 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus-interactive border-r border-gray-300"
                            ...
                            <button
                              className="p-2 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus-interactive border-l border-gray-300"
                              onClick={() => updateQuantity(item.cartItemId, -1)}
                        ...
                        {hasStockIssue && stockInfo && (
                          <div className="flex items-center justify-between pt-1">
                            <span className="font-mono text-xs uppercase text-gray-500 tracking-wider">Actions</span>
          ...
          <div className="border-t bg-white flex-shrink-0" style={{minHeight: '250px', maxHeight: '300px'}}>
            <div className="p-5 space-y-3">
              {/* Subtotal */}
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-mono text-xs uppercase text-gray-500 tracking-wider">Subtotal</span>
                  <span className="font-mono font-semibold text-lg">{totalFormatted}</span>
                </div>
                <p className="font-mono text-xs uppercase tracking-wide text-gray-500 text-center">
              ...
              <button
                ...
                className="w-full bg-black text-white py-3 px-4 font-mono rounded-sm font-semibold uppercase text-sm tracking-wide transition-all duration-200 hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-black focus-ring flex items-center justify-between group"
              ...
              <button
                onClick={onClose}
                className="w-full font-mono text-xs uppercase text-gray-600 hover:text-black transition-colors tracking-wider py-2 mt-2"
              >
                CONTINUE BROWSING
              </button>
            </div>
